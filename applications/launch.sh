...
#PBS -l select=4:ncpus=64:ngpus=4:mem=480GB
#PBS -q main
#PBS -j oe
#PBS -k eod
...
# Launch MPIs
CUDA_VISIBLE_DEVICES="${CUDA_DEVICES}" mpiexec -n $num_nodes --ppn 1 torchrun --nnodes=$num_nodes --nproc-per-node=$num_gpus --rdzv-backend=c10d --rdzv-endpoint=$head_node_ip applications/trainer_single.py -c config/model.yml

#CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 mpiexec -n 2 --ppn 1 -cpu-bind none torchrun --nnodes=2 --nproc-per-node=4 --rdzv-backend=c10d --rdzv-endpoint=$head_node_ip ../../applications/train_fsdp.py -c model.yml