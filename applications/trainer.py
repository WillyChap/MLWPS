...
def trainer(rank, world_size, conf, trial=False):
    ...
    valid_batch_size = config['trainer']['valid_batch_size']
    ...
    apply_grad_penalty = config['trainer']['apply_grad_penalty']
    ...
    vq_codebook_dim = config['model']['vq_codebook_dim']
    vq_codebook_size = config['model']['vq_codebook_size']
    vq_entropy_loss_weight = config['model']['vq_entropy_loss_weight']
    vq_diversity_gamma = config['model']['vq_diversity_gamma']
    discr_layers = config['model']['discr_layers']
    ...
    elif config["trainer"]["mode"] == "ddp":
        model = DDP(clip, device_ids=[device], output_device=None)
    else:
        model = vae
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for steps in batch_group_generator:
            ...
            if vae.discr is not None:
                for _ in range(grad_accum_every):
                    ...
                    with autocast(enabled=amp):
                        loss = vae(img, target, return_discr_loss=True)
            ...
            results_dict["train_loss"].append(batch_loss[0])
        ...
        with torch.no_grad():
            # set up a custom tqdm
            valid_batches_per_epoch = len(test_loader)
            batch_group_generator = tqdm.tqdm(enumerate(test_loader), total=len(test_loader), leave=True)

            valid_loss = []
            for k, (img, (target1, target2)) in batch_group_generator:
                    # move to device
                    img = img.to(device)
                    target = target1.to(device)
