...
# Launch MPIs
CUDA_VISIBLE_DEVICES="${CUDA_DEVICES}" mpiexec -n $num_nodes --ppn 1 --cpu-bind none torchrun --nnodes=$num_nodes --nproc-per-node=$num_gpus --rdzv-backend=c10d --rdzv-endpoint=$head_node_ip applications/trainer_vit3d.py -c config/model.yml
